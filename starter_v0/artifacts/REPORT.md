...
|---|---|---|---|
| **Scenario 1:** Tìm tin tức & tổng hợp digest | `lookup` -> `format` | Ở v0, `lookup`/`social_search` gộp "news" vào query (case R13). Từ v1, agent phân tách đúng query="AI Agent", topic="news", timeframe="day", rồi gọi `format` với đúng 5 item thật vừa lấy được (không bịa item). | `transcripts/v3_openrouter_20260729T174835175599.transcript.json` |
...
| **Scenario 3:** Confirmation Boundary | `clarify(response_type="yes_no")` -> `send` | Ở v0, agent tự ý gọi `send`/tự soạn trả lời cho câu ngoài phạm vi mà không hỏi (case R08/R14). Từ v2/v3, agent bắt buộc dừng lại hỏi xác nhận `yes_no` trước khi gọi `send` hoặc `save_digest`. | `transcripts/v3_openrouter_20260729T190016648733.transcript.json` |
...
| **v1** | `system_prompt.md` + `tools.yaml` | v0 fail 6/20 (R08,R10,R11,R12,R13,R14) — yêu cầu clarify khi thiếu handle/URL, phân tách rõ query/topic/timeframe, nêu rõ phạm vi để không tự gửi câu trả lời ngoài scope | case_accuracy | 0.70 | 0.95 (19/20) — fix 5/6 case (R08,R10,R11,R13,R14); còn R12 (confirmation boundary) để lại v2 | `runs/v1_B_base_openrouter_20260729T155212947469.json` |
...
|---|---|---|---|---|
| **R13 (v0)** | wrong_arg_value | `lookup(query="AI news")` + `social_search(query="AI")` | Gộp từ khóa "news" vào `query` thay vì đưa vào tham số `topic="news"` (2 tool đúng nhưng args sai). | Sửa `system_prompt` & `tools.yaml` hướng dẫn phân tách `query` chỉ chứa từ khóa chính. |
...
| **R11 (v0)** | missing_info | `fetch(url="https://example.com/article")` | Agent tự **bịa ra 1 URL giả** khi người dùng nói "bài viết này" mà chưa đưa link. | Sửa prompt bắt buộc gọi `clarify` khi thiếu `url`, không tự đoán/bịa. |
| **R08/R14 (v0)** | out_of_scope | `send(text="...")` | Agent tự động soạn câu trả lời (toán, code) rồi **tự gọi `send` để gửi luôn**, dù câu hỏi ngoài phạm vi research/social và chưa có xác nhận nào. | Thêm rule "out-of-scope thì từ chối, không gọi tool nào" + rule confirmation boundary trước `send` (v1 fix phạm vi; v2 fix boundary triệt để). |
...
|---|---|---|---|---|
| **Multi-turn Research (Turn 1)** | v3 | `lookup(query="AI Agent", topic="news", timeframe="day")` | `transcripts/v3_openrouter_20260729T174835175599.transcript.json` | Lấy 5 tin tức AI Agent thật thành công |
| **Multi-turn Research (Turn 2)** | v3 | `format(items=[<5 item thật lấy từ turn 1>], template="brief", headline="Tin Tức Nổi Bật Về AI Agent Hôm Nay")` | `transcripts/v3_openrouter_20260729T174835175599.transcript.json` | Tổng hợp digest từ đúng dữ liệu turn 1, không bịa item |
| **Boundary Confirmation (Turn 1)** | v3 | `clarify(question="Bạn có chắc chắn muốn gửi bản tóm tắt tin tức AI hôm nay qua Telegram cho sếp không?", response_type="yes_no")` | `transcripts/v3_openrouter_20260729T190016648733.transcript.json` | `status=waiting_for_user`, tạm dừng chờ user xác nhận trước khi gọi `send` |
...
8. **Bằng chứng "trông thật" chưa chắc là thật — phải verify được từng field**:
   - 2 transcript demo ban đầu (`multiturn_research`, `boundary_confirmation`) trông đúng định dạng nhưng có `tool_calls`/`tool_results`/`tool_events` **không khớp nhau trong cùng 1 file** (ví dụ `format` được model gọi với 1 item bịa "Gemini updates"/example.com, nhưng `tool_results` lại ghi `items: []`, và markdown trả về là `"# ..."` — sai định dạng thật của `render_digest(template="daily_ai_vn")`, vốn luôn dùng `**bold**` và trả đủ 4 field `tool/template/markdown/item_count`). Kết luận: 2 file này bị dựng tay, không phải output thật từ hệ thống.
   - Đã tái hiện lại đúng 2 kịch bản qua server thật, thay bằng transcript thật (`v3_openrouter_20260729T174835175599`, `v3_openrouter_20260729T190016648733`) trước khi nộp. Bài học: khi review evidence của đồng đội, không chỉ tin "có transcript là đủ" — phải đối chiếu `tool_calls` với `tool_results` có nhất quán không, và so khớp với schema thật của tool trong `tools/<name>/tool.py`.
